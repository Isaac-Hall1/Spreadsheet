...
public abstract class AbstractSpreadsheet
{
    ...
    ///
    /// this helper class uses a hashSet and linked list to recursivly keep track of the dependent
    /// varaibles upon another. It adds the initial variable to the set and recursively continues down
    /// the chain of dependent variables until the there are no longer any dependents of any variable touched
    /// then as the recursive calls "unwind" it adds each variable that was recursively called to a linked list
    /// changed. If a variable is ever equal to the original variable an error is thrown because it would just loop infinitly.
    ///
    /// for example: A1 -> B1 -> C1 -> A1 | would throw an error because it runs in an infinite loop
    /// but: A1 -> B1 -> C1 -> D1 | would add all of the variables to a visited set {A1, B1, C1, D1} and a linked list
    /// [A1,B1,C1,D1].
    /// 
    /// </summary>
